...
char* indexes(const char* path)
{
    ...
    if (path != NULL) 
    {
        ...
        if (file != NULL) 
        {
            return directory = "/path/to/a/directory/index.php";
        }
...
{
    // TODO
    // Store the file extension name in a variable
    char* extension = strrchr(path, '.');
    
    // if it exists, in case strrchr doesn't really find anything
    if(extension != NULL)
    {
        // compare and match the extension with the return type
        // using strcasecmp because it will overlook capitaliztion when searching for a match
